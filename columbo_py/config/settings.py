"""Typed, centralized configuration for Columbo.
...
there is a single source of truth for what Columbo's knobs are and what they
default to.

Resolution cascade (last wins):

1. ``config/defaults.toml`` shipped in the package.
2. An optional user override file at ``$COLUMBO_CONFIG`` (a TOML with just the
   keys you want to change - deep-merged over the defaults).
3. The documented ``COLUMBO_*`` environment variables, so the overrides the
...
# COLUMBO_* env vars that override a specific config path. Each maps to
...
_ENV_OVERRIDES: tuple[tuple[str, str, str, Callable[[str], Any]], ...] = (
    ("COLUMBO_CACHE_DIR", "cache", "dir", str),
    ("COLUMBO_SEARCH_TTL_S", "cache", "search_ttl_s", int),
    ("COLUMBO_LOOKUP_TTL_S", "cache", "lookup_ttl_s", int),
    ("COLUMBO_SCRAPE_TTL_S", "cache", "scrape_ttl_s", int),
    ("COLUMBO_DICTIONARY_URL", "dictionary", "url", str),
    ("COLUMBO_DICTIONARY_DOMAIN", "dictionary", "domain", str),
    ("COLUMBO_SLACK_TOKEN_PATH", "sources.slack", "token_path", str),
    ("COLUMBO_DRIVE_TOKEN_PATH", "sources.drive", "token_path", str),
)


def _apply_env_overrides(data: dict[str, Any], environ: dict[str, str]) -> None:
    """Mutate ``data`` in place, folding documented COLUMBO_* env vars over it."""
...
def _load_defaults() -> dict[str, Any]:
    text = resources.files("columbo_py.config").joinpath("defaults.toml").read_text("utf-8")
    return tomllib.loads(text)


def load_settings(environ: dict[str, str] | None = None) -> Settings:
    """Build a :class:`Settings` from the shipped defaults, an optional
    ``$COLUMBO_CONFIG`` override file, and the documented ``COLUMBO_*`` env
    vars (in that precedence, last wins)."""
    env = os.environ if environ is None else environ
    data = _load_defaults()

    override_path = env.get("COLUMBO_CONFIG", "").strip()
    if override_path:
...
# Process-wide singleton. Consumers do ``from columbo_py.config import SETTINGS``.
