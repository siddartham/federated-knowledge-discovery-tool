...
as an ordinary Columbo `SearchSource`, so it plugs into the existing planner /
...
Config (env): COLUMBO_MCP_TESTING_URL (server URL; unset => source disabled),
COLUMBO_MCP_TESTING_TOKEN (optional per-user bearer token for RBAC at source).
...
from columbo_py.sources.mcp.client import MCPClient
...
class MCPTestingPlatformSource:
    ...
    async def initialize(self) -> None:
        if self._client is None:
            ...
            if not url:
                raise RuntimeError(
                    "Testing Platform MCP not configured: set COLUMBO_MCP_TESTING_URL to the "
                    "server URL (and COLUMBO_MCP_TESTING_TOKEN for a per-user bearer token)."
                )
            from columbo_py.sources.mcp.client import RemoteMCPClient
