...
from columbo_py.sources.mcp import FakeMCPClient, MCPTestingPlatformSource, ToolContent
...
async def test_unconfigured_source_errors_clearly(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("COLUMBO_MCP_TESTING_URL", raising=False)
    src = MCPTestingPlatformSource()  # no injected client, no env
    with pytest.raises(RuntimeError, match="COLUMBO_MCP_TESTING_URL"):
        await src.initialize()
