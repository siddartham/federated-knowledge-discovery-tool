...
from columbo_py.sources.mcp import FakeMCPClient, MCPToolProvider, ToolContent
...
class _OneDocSource:
    ...
    async def search(self, query: str, opts: Options) -> list[Result]:
        return [Result(source="demo", id="1", title="t", content="Columbo answers questions.",
                       permalink="demo://1")]
...
async def test_run_result_carries_selected_contexts(tmp_path: Path) -> None:
    ...
    assert "Columbo" in result.contexts[0]["content"]
