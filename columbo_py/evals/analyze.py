"""Offline analysis of Columbo run logs — validates the scoring / confidence design
against real runs, read-only.

Consumes the JSONL event logs written by ``EventEmitter`` (in ``~/.columbo/runs``)
...
from columbo_py.engine.orchestrator.models import MAX_SCORE, scored_composite
...
def format_report(r: dict[str, Any]) -> str:
    out: list[str] = []
    out.append(f"Columbo run-log analysis — {r['runs']} run(s), "
