"""Chainlit chat front-end for Columbo.

Launch with `columbo ui` (or `chainlit run columbo_py/ui/app.py`); requires the
...
from columbo_py.infra.events.emitter import configure_logging
from columbo_py.ui.events import UIEvent, UIEventEmitter
from columbo_py.ui.session import WarmSession
...
async def on_chat_start() -> None:
    ...
    await session.start()
    cl.user_session.set("columbo", session)
    await cl.Message(
...
async def on_message(message: cl.Message) -> None:
    session: WarmSession = cl.user_session.get("columbo")
...
async def on_chat_end() -> None:
    session = cl.user_session.get("columbo")
