...
![Match the auth flow to the deployment](images/auth/dossier-auth-flow-first-principle.svg)
...
The browser is already SSO-logged-in; Dossier drives *that* session, so every resource sees the actual user.
