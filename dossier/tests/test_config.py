...
the documented DOSSIER_* env vars must still win over the file.
"""
...
from dossier.config.settings import load_settings
...
def test_env_vars_override_the_file() -> None:
    env = {
        "DOSSIER_CACHE_DIR": "/tmp/dossier-test-cache",
        "DOSSIER_SEARCH_TTL_S": "60",
        "DOSSIER_DICTIONARY_URL": "https://dict.internal",
        "DOSSIER_DICTIONARY_DOMAIN": "networking",
        "DOSSIER_SLACK_TOKEN_PATH": "/tmp/slack.json",
    }
    s = load_settings(environ=env)

    assert s.cache.dir == Path("/tmp/dossier-test-cache")
...
def test_dossier_config_file_deep_merges(tmp_path: Path) -> None:
    """A user DOSSIER_CONFIG overrides only the keys it names; the rest fall
    ...
    s = load_settings(environ={"DOSSIER_CONFIG": str(override)})
...
def test_env_var_wins_over_dossier_config_file(tmp_path: Path) -> None:
    override = tmp_path / "override.toml"
    override.write_text("[cache]\nsearch_ttl_s = 111\n", encoding="utf-8")
    s = load_settings(
        environ={"DOSSIER_CONFIG": str(override), "DOSSIER_SEARCH_TTL_S": "222"}
    )
