...
- `demo`    the real `ask` pipeline with a canned default question, for
            showing someone Dossier without them typing anything.
...
from dossier.infra.cache.store import (
    CACHE_NAMESPACES,
    DEFAULT_CACHE_ROOT,
    CacheStore,
)
...
class _DemoSource:
    ...
    _DOCS = [
        Result(
            ...
            content=(
                "Dossier is a CLI agent that answers questions across Slack, "
        ...
        Result(
            ...
            content=(
                "Dossier runs entirely on the user's machine via a persistent "
    ...
    def kind(self) -> str:
        return "An in-memory demo source with a couple of canned documents about Dossier itself."
...
def smoke() -> None:
    ...
    plan = json.dumps(
        {
            ...
            "actions": {
                "searches": [{"source": "demo", "query": "what is dossier"}],
                "scrapes": [],
                "lookups": [],
            },
        }
    ...
    synth = json.dumps(
        {
            "answer": (
                "Dossier is a CLI agent for cross-source Q&A [demo:1], "
...
def clear_cache(
    ...
    """Clear Dossier's on-disk caches. By default clears all four namespaces
...
def evaluate(
    ...
    Dossier's own Claude client + response cache (so re-runs are deterministic
    and free). Needs only ANTHROPIC_API_KEY - no sources, no browser. This is a
    separate offline gate; it never runs inside `ask`."""
    from dossier.engine.llm.claude import ClaudeClient
    from dossier.evals import evaluate_sample, load_samples
...
def demo(question: str = typer.Argument(DEMO_QUESTION)) -> None:
    ...
    a canned default question - for showing Dossier to someone without them
    needing to think of a question first."""
    from dossier.cli.main import _ask
    from dossier.cli.main import _print_result as _print
...
def analyze(
    ...
    from dossier.evals.analyze import (
        build_report,
        format_report,
        load_evals_index,
        load_events,
